...
static const SHELL_IODesc ios[] = { { &SHELL_stdio, SHELL_DefaultShellBuffer,
...
#if PL_CONFIG_HAS_ZORK_
	else if (UTIL1_strcmp((char*)cmd, CLS1_CMD_STATUS) == 0
			|| UTIL1_strcmp((char*)cmd, "zork") == 0) {
		*handled = TRUE;
		startZorkTask();
		return ERR_OK;
	}
...
static void ShellTask(void *pvParameters) {
	...
	for (;;) {
...
#elif PL_CONFIG_HAS_SHELL_QUEUE /* !PL_CONFIG_SQUEUE_SINGLE_CHAR */
		{
			...
			// Somebody must take the things in the queues
...
#if PL_CONFIG_HAS_RTOS
	if (xTaskCreate(ShellTask, "Shell", 900 / sizeof(StackType_t), NULL,
			tskIDLE_PRIORITY + 1, &ShellHandler) != pdPASS) {
		for (;;) {
		} /* error */
	}
#endif
